...
def relatively_ensemble_NB(x_train, y_train, x_test, y_test, cnt) -> List:
	'''
	Ensemble KNN
	:param x_train:训练数据集
	:param y_train:训练标签
	...
	for i in range(cnt):
		# 采样
		sub_x_train, sub_y_train = random_sub_sample(x_train, y_train, 0.25 + i * 0.02)
		# 训练
		# print("开始第 %d 次训练" % (i + 1))
...
if __name__ == "__main__":
	...
	for i in range(5, 35, 5):
		pre = relatively_ensemble_NB(x_train, y_train, x_test, y_test, i)
		end = time.time()
	...
	# pre = relatively_ensemble_NB(x_train, y_train, x_test, y_test, 10)
	# end = time.time()
	# errArray = np.mat(np.ones((len(y_test), 1)))
	# print("正确率为：%.3f %%" % ((errArray[pre == y_test].sum() / len(pre)) * 100))
