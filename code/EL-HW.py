...
def relatively_ensemble_KNN(x_train, y_train, x_test, y_test, cnt) -> List:
	'''
	Ensemble KNN
	:param x_train:训练数据集
	:param y_train:训练标签
	...
	for i in range(cnt):
		...
		model = [neighbors.KNeighborsClassifier(), svm.SVC(), naive_bayes.MultinomialNB(),
		         tree.DecisionTreeClassifier()]
		model[choice].fit(sub_x_train, sub_y_train)
		# print("第 %d 次训练测试集得分：" % (i + 1), model[choice].score(x_test, y_test))
		pre_of_x_test.append(model[choice].predict(x_test))
...
def ensemble_wKNN(self) -> List:
	'''
	采用带权集成
	:return: 带权的弱分类器
	'''
	errArray, pre_of_x_test, finally_pre = [], [], []
	for i in range(self.cnt):
		# 采样
		sub_x_train, sub_y_train = self.random_sub_sample()
		# print(sub_x_train, sub_x_train.shape)
		# print(sub_y_train, sub_y_train.shape)
		# 训练
		model = KNN(sub_x_train, sub_y_train, self.x_test, self.y_test, self.k)
		pre_of_x_test.append(model.test())
	...
	for i in range(self.cnt):
		errCnt = 0
		for j in range(len(pre_of_x_test[i])):
			if pre_of_x_test[i][j] != self.y_test[j]:
				errCnt += 1
		errArray.append(errCnt / len(self.y_test))

	# 错误率归一化
	errArray = np.array(errArray)
	errArray = errArray / errArray.sum()

	# 集成
	for i in range(len(pre_of_x_test[0])):
		vector = pre_of_x_test[:, i]
		vector = np.array(vector)
		finally_element = (vector * errArray).sum()
		if finally_element - int(finally_element) >= 0.5:
			finally_pre.append(int(finally_element) + 1)
		else:
			finally_pre.append(int(finally_element))
...
if __name__ == "__main__":
	...
	for i in range(5, 35, 5):
		pre = relatively_ensemble_KNN(x_train, y_train, x_test, y_test, i)
		end = time.time()
	...
	# pre = relatively_ensemble_KNN(x_train, y_train, x_test, y_test, 10)
	# end = time.time()
	# errArray = np.mat(np.ones((len(y_test), 1)))
	# print("正确率为：%.3f %%" % ((errArray[pre == y_test].sum() / len(pre)) * 100))
