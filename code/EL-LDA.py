...
def relatively_ensemble_LDA(x_train, y_train, x_test, y_test, cnt) -> List:
	'''
	Ensemble KNN
	:param x_train:训练数据集
	:param y_train:训练标签
...
if __name__ == "__main__":
	...
	for i in range(5, 35, 5):
		pre = relatively_ensemble_LDA(x_train, y_train, x_test, y_test, i)
		end = time.time()
	...
	# pre = relatively_ensemble_LDA(x_train, y_train, x_test, y_test, 10)
	# end = time.time()
	# errArray = np.mat(np.ones((len(y_test), 1)))
	# print("正确率为：%.3f %%" % ((errArray[pre == y_test].sum() / len(pre)) * 100))
